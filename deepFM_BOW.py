    ...
    with open(file, 'r') as rd:
        while True:
            ...
            for word in words:
                ...
                if len(tokens[2]) <= 0:
                    tokens[2] = '0'
                cur_feature_list.append([int(tokens[0]), int(tokens[1]), float(tokens[2])])
...
def pre_build_data_cache(infile,batch_size):
    # wt = open(outfile, 'wb')
    # print("preapare....")
...
def single_run(params):
    ...
    _keep_prob = tf.placeholder(tf.float32,shape = [params['layer_num']], name = 'keep_prob')
    ...
    if params['output_predictions']:
        saver.restore(sess, 'models/0.0001-4')
        predict_test_file(preds, sess, params['test_file'], _indices, _values, _shape, _y, _values2, _field2feature_indices, _field2feature_values,_field2feature_weights
                                  , _field2feature_shape,_keep_prob, batch_size, params)
        return
    ...
    for eopch in range(n_epoch):
        ...
        for training_input_in_sp in pre_build_data_cache(params['train_file'],batch_size):
            ...
            iteration += 1
            glo_ite += 1
            _,cur_los = sess.run([train_step,  loss], feed_dict={
                ...
                ,_keep_prob:params['train_keep_prob']
            })
            time_cp02 = clock()

            time_sess += time_cp02 - time_cp01
            train_loss_per_epoch += cur_los
            if iteration%100 == 0:
                print("current iteration :"+str(iteration) + " train loss is :"+ str(train_loss_per_epoch/batch_size))
        print("one epcho train end")
        ...
        print('time for eopch ', eopch, ' ', "{0:.4f}min".format((end - start) / 60.0), ' time_load_data:', "{0:.4f}".format(time_load_data), ' time_sess:',
             "{0:.4f}".format(time_sess), ' train_loss: ', train_loss_per_epoch)
        
        if eopch % 1 == 0:
            model_path = params['model_path'] + "/" + str(params['layer_sizes'][0])+'_'+str(params['layer_sizes'][1]) + str(
                params['reg_w_linear']).replace(':', '_')
            os.makedirs(model_path, exist_ok=True)
            saver.save(sess, model_path, global_step=eopch)            
            metrics=predict_valid_file(preds, sess, params['valid_file'], _indices, _values, _shape, _y,
                              _values2, _field2feature_indices, _field2feature_values,_field2feature_weights
                                  , _field2feature_shape, _keep_prob,eopch, batch_size, 'test', model_path, params['output_predictions']
                                  , params)
            metrics_strs = []
            auc = 0
...
def predict_valid_file(preds, sess, test_file, _indices, _values, _shape, _y, _values2, _field2feature_indices, _field2feature_values,_field2feature_weights
                                  , _field2feature_shape, _keep_prob,epoch, batch_size, tag, path, output_prediction, params):
    ...
    query2res = {}
    for test_input_in_sp in pre_build_data_cache(test_file,batch_size):
        predictios = sess.run(preds, feed_dict={
            ...
            _shape: test_input_in_sp['shape'], _y: test_input_in_sp['labels'], _values2: test_input_in_sp['values2'],
                ...
                , _keep_prob:params['test_keep_prob']
        }).reshape(-1).tolist()
        ...
        if output_prediction:
            ...
            qids = [i for i in range(batch_size)]
            for (gt, preded, qid) in zip(test_input_in_sp['labels'].reshape(-1).tolist(), predictios, qids):
...
def predict_test_file(preds, sess, test_file, _indices, _values, _shape, _y, _values2, _field2feature_indices, _field2feature_values,_field2feature_weights
    ...
    with open(contest_dir+'074042.csv','w') as f:
        f.writelines(lines)
...
def compute_metric(query2res, params):
    ...
    for m in params['metrics']:
        ...
        if 'logloss' in m['name']:
            ...
            #print('gt_scores ',gt_scores) 
            #print('pred_scores ',pred_scores)   
            result['log_loss'] = loss = log_loss(np.asarray(gt_scores), np.asarray(pred_scores))
        
    return result


def build_model(_indices, _values, _values2, _shape, _field2feature_indices, _field2feature_values,_field2feature_weights, _field2feature_shape, _y,_keep_prob, params):
    ...
    if params['is_use_dnn_part']:
        ...
        for layer_size in layer_sizes:
            ...
            cur_hidden_nn_layer = tf.nn.dropout(cur_hidden_nn_layer, keep_prob =  _keep_prob[layer_idx])
...
def run():
    ...
    params = {
        ...
        'dim': 15,
        'model_path': 'models',
        'train_file':  contest_dir + '/gbm/ffm_train',
        'valid_file':   contest_dir + '/gbm/ffm_valid' ,
        'test_file': contest_dir + '/gbdtfeature/feature_30/ffm_data_test_30',
        ...
        'test_keep_prob' : [1,1,1],
        'train_keep_prob': [0.8,0.8,0.8],
        'activations':['relu','relu','relu'],
        'layer_sizes': [400,400,400],
        'layer_num':3
    }
   
    # pre_build_data_cache(params['train_file'], "", params['batch_size'])
...
if __name__ == '__main__':
    ...
    # field_index_dic = read_dic(construct_feature_dir+'middle/lda_field_index_dic')
    # feature_index_dic = read_dic(construct_feature_dir+'middle/lda_feature_index_dic')
    FIELD_COUNT =  30#30 #46
    feature2field = read_dic(contest_dir+'/ffm/featureindex_field_dic_1')
    FEATURE_COUNT = len(feature2field)#+30*20 #46
    # res = sorted(field_index_dic.items(),key=lambda x: x[1])
    # FIELD_COUNT = res[-1][1]+1
    # res = sorted(feature_index_dic.items(),key=lambda x: x[1])
    # FEATURE_COUNT = res[-1][1]+1
    print(FIELD_COUNT)
    print(FEATURE_COUNT)
    # FIELD_COUNT = 
    # FEATURE_COUNT = 
